 ...
 * @ingroup themeable
 */
function design_jani_menu_local_task($mid, $active, $primary) {
  $active_class = "";
  if ($active) {
    $active_class .= "active ";
  }
 ...
 *   The url of the feed.
 */
function design_jani_feed_icon($url) {
 ...
 */
function design_jani_comment_wrapper($content, $type = null) {
  static $node_type;
 ...
 */
function design_jani_breadcrumb($breadcrumb) {
  if (!empty($breadcrumb)) {
    return '<div class="breadcrumb art-postcontent">'. implode(' | ', $breadcrumb) .'</div>';
  }
}

function design_jani_service_links_node_format($links) {
 ...
 * @ingroup themeable
 */
function design_jani_button($element) {
...
*/
function design_jani_img_assist_page($content, $attributes = NULL) {
